#include <math.h>
#include <malloc.h>
//2D arrays are stored as 1D arrays and this macro is used to index them
//I index them this way as opposed to i*N+j so that I can treat columns as
//vectors in place.  I need to do this more often that I ned to do it with rows.
//Do not change this.  I rely on this storage strategy in hessreduce.
...
void printMat(Complex *M, int n)
{
	...
	for(i = 0; i < n; i++)
	{
		for(j = 0; j < n; j++)
		{
			printf("%1.8f + %1.8fi,\t", INDEX(M,n,i,j).real, INDEX(M,n,i,j).imag);
		}
		printf(";\n");
	}
	printf("]\n");
...
void copyVect(Complex *src, Complex *dest, int n)
{
	int i;
	for(i = 0; i < n; i++)
	{
...
*/
void rotapp(Real *c, Complex *s, Complex *x, Complex *y, Complex *t, int n)
{
	int i;
...
*/
void backsearch(Complex *H, int n, int l, int *i1, int *i2, Real epsilon)
{
	//2. i1 = i2 = l
	*i1 = l;
	*i2 = l;
...
void run_test()
{
	...
	double val;
	FILE *file; 
	file = fopen("my_custom_zgeev_times_on_cpu.txt","w"); /* append file (add text to 
